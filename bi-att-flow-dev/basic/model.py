...
from my.tensorflow.rnn import dropout, dense  # cudnn_gru
...
class Model(object):
    def __init__(self, config, scope, rep=True):
        ...
        N, M, JX, JQ, VW, VC, W = \
            config.batch_size, config.max_num_sents, config.max_sent_size, config.max_ques_size, \
    ...
    def _build_forward(self):
        ...
        with tf.variable_scope("prepro"):
            # [N, J, d], [N, d] the first modification for gru cell
        ...
        with tf.variable_scope("main"):
            if config.dynamic_att:
                ...
                p0 = attention_layer(config, self.is_train, h, u, h_mask=self.x_mask, u_mask=self.q_mask, scope="p0",
                                     tensor_dict=self.tensor_dict)  # [N M JX xd]

            # self attention
            with tf.variable_scope("match"):
                self_att = dot_attention(p0, p0, mask=self.x_mask, hidden=d, keep_prob=config.keep_prob,
                                         is_train=self.is_train)
                # rnn = cudnn_gru(num_layers=1, num_units=d, batch_size=N, max_num_sents=M,
                #                input_size=self_att.get_shape().as_list()[-1], keep_prob=config.keep_prob,
                #                is_train=self.is_train)
                # match = rnn(self_att, seq_len=self.x_len)
                match = self_att
            ...
            (fw_g0, bw_g0), _ = bidirectional_dynamic_rnn(first_cell_fw, first_cell_bw, match, x_len, dtype='float',
                                                          scope='g0')  # [N, M, JX, 2d]
            ...
            logits = get_logits([g1, match], d, True, wd=config.wd, input_keep_prob=config.input_keep_prob,
            ...
            (fw_g2, bw_g2), _ = bidirectional_dynamic_rnn(d_cell4_fw, d_cell4_bw,
                                                          tf.concat(axis=3, values=[match, g1, a1i, g1 * a1i]),
                                                          x_len, dtype='float', scope='g2')  # [N, M, JX, 2d]
            g2 = tf.concat(axis=3, values=[fw_g2, bw_g2])
            logits2 = get_logits([g2, match], d, True, wd=config.wd, input_keep_prob=config.input_keep_prob,
                                 mask=self.x_mask,
    ...
    def _build_loss(self):
        ...
        if config.wy:
            ...
            dampened_losses = losses * (
                    (tf.cast(self.x_mask, 'float') - tf.cast(self.wy, 'float')) * damp_ratio + tf.cast(self.wy,
                                                                                                       'float'))
...
def dot_attention(inputs, memory, mask, hidden, keep_prob=1.0, is_train=None, scope="dot_attention"):
    with tf.variable_scope(scope):
        ...
        with tf.variable_scope("attention"):
            ...
            outputs = tf.matmul(inputs_, tf.transpose(memory_, [0, 1, 3, 2])) / (hidden ** 0.5)
            mask = tf.tile(tf.expand_dims(mask, axis=1), [1, 1, JX, 1])

            logits = tf.nn.softmax(softmax_mask(outputs, mask))
            outputs = tf.matmul(logits, memory)
            res = tf.concat([inputs, outputs], axis=3)
