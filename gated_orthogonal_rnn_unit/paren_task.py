...
def main(model, T, n_iter, n_batch, n_hidden, capacity, comp, fft):
    ...
    V_weights = tf.get_variable("V_weights", shape=[n_hidden, n_classes * n_output], dtype=tf.float32,
                                initializer=tf.random_uniform_initializer(-V_init_val, V_init_val))
    V_bias = tf.get_variable("V_bias", shape=[n_classes * n_output], dtype=tf.float32,
                             initializer=tf.constant_initializer(0.01))
    hidden_out_list = tf.unstack(hidden_out, axis=1)
    temp_out = tf.stack([tf.matmul(i, V_weights) for i in hidden_out_list])
    output_data = tf.reshape(tf.nn.bias_add(tf.transpose(temp_out, [1, 0, 2]), V_bias),
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Parenthesis Task")
