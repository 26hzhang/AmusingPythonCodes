...
limit = {'maxw': 16, 'minw': 5, 'maxph': 16, 'minph': 5}
...
def index_phonemes(phoneme_lists):
    phoneme_vocab = set([phoneme for phonemes in phoneme_lists for phoneme in phonemes])
...
def filter_data(words, phoneme_lists):
    ...
    for word, phonemes in zip(words, phoneme_lists):
        if word.isalpha():
            if limit['maxw'] > len(word) > limit['minw']:
                if limit['maxph'] > len(phonemes) > limit['minph']:
...
def zero_pad(words, phoneme_lists, alpha2idx, pho2idx):
    ...
    for i in range(data_len):
        ...
        word_indices = [alpha2idx[alpha] for alpha in words[i]] + [0]*(limit['maxw'] - len(words[i]))
        pho_indices = [pho2idx[phoneme] for phoneme in phoneme_lists[i]] + [0]*(limit['maxph'] - len(phoneme_lists[i]))
...
def process_data():
    ...
    data_ctl = {'idx2alpha': idx2alpha, 'idx2pho': idx2pho, 'pho2idx': pho2idx, 'alpha2idx': alpha2idx, 'limit': limit}
