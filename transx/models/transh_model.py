...
class TransH(object):
    def __init__(self, entity_size, relation_size, hidden_size, margin=1.0, learning_rate=0.001, l1_flag=True,
                 model_name='transh_model', ckpt_path='./ckpt/transh/'):
        ...
        with tf.name_scope("embedding"):
            self.ent_embeddings = tf.get_variable(name="ent_embedding", shape=[self.entity_size, hidden_size],
                                                  initializer=tf.contrib.layers.xavier_initializer(uniform=False))
            self.rel_embeddings = tf.get_variable(name="rel_embedding", shape=[self.relation_size, hidden_size],
                                                  initializer=tf.contrib.layers.xavier_initializer(uniform=False))
            self.normal_vector = tf.get_variable(name="normal_vector", shape=[self.relation_size, hidden_size],
                                                 initializer=tf.contrib.layers.xavier_initializer(uniform=False))
