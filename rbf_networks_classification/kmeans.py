...
class KMeans(object):
    ...
    def _randomInitCenter(self, data):
    ...
    def updateCluster(self, data, centers):
        '''updating clusters'''
    ...
    def updateCenter(self, data, nearest):
        '''updating centroid'''
    ...
    def train(self, data):
        ...
        initcenters = self._randomInitCenter(data)
        # first iter
        nearest, loss = self.updateCluster(data, initcenters)
        centers =  self.sess.run(self.updateCenter(data, nearest))
        lastloss = self.sess.run(loss)
        for i in range(self._max_iter):
            # updating nearest_indices (cluster) and orthocenter alternately
            nearest_indices, loss=self.updateCluster(data, centers)
            centers = self.sess.run(self.updateCenter(data, nearest_indices))
            lossvalue = self.sess.run(loss)
            print 'iter:', i, 'loss:', lossvalue
            if lastloss - lossvalue < 0.00001:
                break
            lastloss = lossvalue
        print('finsied')
