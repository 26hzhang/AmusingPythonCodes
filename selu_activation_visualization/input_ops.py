...
def create_input_ops(dataset, batch_size, num_threads=1, is_training=False, data_id=None, scope='inputs', shuffle=True):
    ...
    with tf.device("/cpu:0"), tf.name_scope(scope):
        input_ops['id'] = tf.train.string_input_producer(tf.convert_to_tensor(data_id), capacity=128)\
        ...
        input_ops['id'], input_ops['image'], input_ops['label'] = tf.py_func(load_fn, inp=[input_ops['id']],
    ...
    if shuffle:
        batch_ops = tf.train.shuffle_batch(input_ops, batch_size=batch_size, num_threads=num_threads, capacity=capacity,
                                           min_after_dequeue=min_capacity)
    else:
        batch_ops = tf.train.batch(input_ops, batch_size=batch_size, num_threads=num_threads, capacity=capacity)
